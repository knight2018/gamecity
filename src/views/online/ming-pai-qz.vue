...
<script>
    ...
    import { MingPaiQiangZhuangColumns} from './data/define.js';

    export default {
       components: { PageablePlayerList },
        name: 'online-mpqz-player-list',
        data () {
            return {
                params: {
                    columnList: [],
        ...
        created () {
            var cols = []
            for (let i = 0; i < MingPaiQiangZhuangColumns.length; i++) {
                cols.push(MingPaiQiangZhuangColumns[i])
            }
            cols.splice( 13, 0,{
                title: '操作',
                key: 'operation',
