...
<script>
    ...
    import { mingchePlayerColumns } from './define'

    export default {
        components: { PageablePlayerList },
        name: 'mingche-player-control',
        data () {
            return {
               params: {
                    columnList: [],
        ...
        created () {
            let cols = [];
            for (let i = 0; i < mingchePlayerColumns.length; i++) {
                cols.push(mingchePlayerColumns[i]);
            }
            cols.splice( 13, 0,{
                title: '操作',
                key: 'operation',
        ...
        methods: {
            getPlayerList(){
             let _this= this;
             let url= api.URL.Mingche_OnlineUser_List(this.params.currentPage, this.params.pageSize, this.params.orderByDesc, this.params.orderByField);
             api.getData(this, url, function (data) {
                let datas= data.datas;
                for (var index = 0; index < datas.length; index++) {
                    var playerInfo = datas[index];
                    var offline = playerInfo['offline'];
                    if (offline == false) {
