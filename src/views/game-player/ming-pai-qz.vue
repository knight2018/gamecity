...
<script>
    ...
    export default {
        ...
        created () {
            var cols = []
            for (let i = 0; i < mingPaiPlayerColumns.length; i++) {
                cols.push(mingPaiPlayerColumns[i])
            }
            // cols.push({
            //     title: '监控中心2',
            //     key: 'monitor'
        ...
        methods: {
            showMonitor () {
                ...
                onOk: () => {
                    api.postData(this, api.URL.MingPai_Update_Player_Winloss, {userId: this.userId, winLossRate: this.winLossRate, winLoss: this.winLoss}, data => {
            ...
            getTableInfo(){
                let _this = this;
                    let url= api.URL.MingPai_PlayerList(this.params.currentPage, this.params.pageSize, this.params.orderByDesc, this.params.orderByField);
